...
export const sendOtpSMS = async (phoneNumber, otp, name) => {
  try {
    const response = await axios.post(process.env.SEND_SMS_ENDPOINT, {
      ...
      Is_Unicode: true,
      Is_Flash: true,
      Message: `Dear ${name} Welcome to Rider's Kart. We're excited to have you onboard. Get ready for fast, reliable deliveries at your fingertips. Here is your OTP ${otp}`,
